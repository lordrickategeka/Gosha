        ...
        @foreach ([1 => 'Customer & Vehicle', 2 => 'Job Details', 3 => 'Items', 4 => 'Review'] as $step => $label)
            <div class="flex items-center flex-1 {{ !$loop->last ? '' : 'flex-none' }}">
                <div class="flex flex-col items-center">
                    ...
                    <span class="text-xs mt-1 hidden sm:block {{ $currentStep === $step ? 'text-primary font-medium' : 'text-base-content/50' }}">{{ $label }}</span>
                </div>
                @if(!$loop->last)
        ...
        <div class="card-body">
            ...
            <div class="form-control mb-4">
                <label class="label">
                    ...
                    <button type="button" wire:click="showNewCustomerForm" class="btn btn-ghost btn-xs">+ New Customer</button>
                </label>
                <div class="relative">
                    <input type="text" wire:model.live.debounce.300ms="customerSearch"
                        wire:focus="$set('showCustomerDropdown', true)"
                        placeholder="Search by name or phone..." class="input input-bordered w-full" autocomplete="off" />
            ...
            @if($customer_id)
                <div class="form-control">
                    <label class="label">
                        ...
                        <button type="button" wire:click="showNewVehicleForm" class="btn btn-ghost btn-xs">+ New Vehicle</button>
                    </label>
                    <select wire:model="vehicle_id" class="select select-bordered w-full">
                        ...
                        @foreach($this->vehicles as $vehicle)
                            <option value="{{ $vehicle->id }}">
                                ...
                                @if($vehicle->chassis_number) ({{ $vehicle->chassis_number }})@endif
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="card-body">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="form-control">
                    ...
                    <input type="number" wire:model="mileage_in" placeholder="Current mileage" class="input input-bordered w-full" />
                </div>
                <div class="form-control">
    ...
    <div class="card bg-base-100 shadow-sm">
        <div class="card-body">
                        ...
                        <div class="border border-base-300 rounded-lg p-4 relative">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
                                ...
                                <div class="form-control sm:col-span-2">
                                    ...
                                    @error("items.$index.description") <span class="text-error text-xs">{{ $message }}</span> @enderror
                                </div>
                                <div class="form-control">
                            ...
                            @if(($item['item_type'] ?? '') === 'part')
                                <div class="form-control mb-3">
                                    <label class="label py-1"><span class="label-text text-xs">Inventory Item (optional)</span></label>
                                    <select wire:model="items.{{ $index }}.inventory_item_id" class="select select-bordered select-sm">
                                        <option value="">None</option>
                                        @foreach($this->parts as $part)
                                            <option value="{{ $part->id }}">{{ $part->name }} ({{ $part->sku }}) – Stock: {{ $part->quantity }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            @endif
                            ...
                            @if(!$this->isJobcarder())
                                <div class="form-control mb-3">
                                    ...
                                    <input type="number" wire:model="items.{{ $index }}.unit_price"
                                        step="1" min="0" class="input input-bordered input-sm w-40" />
                                    <span class="text-xs text-base-content/50 mt-1">
                                        Total: UGX {{ number_format(($item['quantity'] ?? 0) * ($item['unit_price'] ?? 0)) }}
                                    </span>
                                </div>
                            @else
                                <p class="text-xs text-base-content/40 italic mb-2">Pricing will be set by the quoter.</p>
                            @endif

                            <div class="form-control">
                                <label class="label py-1"><span class="label-text text-xs">Images (optional)</span></label>
                                <input type="file" wire:model="itemImages.{{ $index }}"
                                    multiple accept="image/*" class="file-input file-input-sm file-input-bordered w-full max-w-sm" />
                                <span wire:loading wire:target="itemImages.{{ $index }}" class="text-xs text-base-content/50">Uploading...</span>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="flex gap-2 mt-4">
                <button type="button" wire:click="addItem('labor')" class="btn btn-outline btn-sm">+ Add Labor</button>
                <button type="button" wire:click="addItem('part')" class="btn btn-outline btn-sm">+ Add Part</button>
            </div>
            ...
            <div class="card-body">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <p class="font-medium">{{ optional(\App\Models\Customer::find($customer_id))->name ?? '—' }}</p>
                    </div>
                    <div>
                        <p class="text-base-content/50 text-xs uppercase mb-1">Vehicle</p>
                        @php $v = \App\Models\Vehicle::find($vehicle_id); @endphp
                        <p class="font-medium">{{ $v ? $v->registration_number . ' ' . $v->make . ' ' . $v->model : '—' }}</p>
                    </div>
                    <div>
                    ...
                    <tbody>
                        @foreach($items as $item)
                            <tr>
                                ...
                                <td>{{ $item['quantity'] }}</td>
                                @if(!$this->isJobcarder())
                                    <td class="text-right">UGX {{ number_format(($item['quantity'] ?? 0) * ($item['unit_price'] ?? 0)) }}</td>
                                @endif
                            </tr>
                        @endforeach
                    </tbody>
