<div>
    ...
    <div class="mb-8">
        <form wire:submit.prevent="{{ $editMode ? 'update' : 'create' }}"
            class="card bg-base-100 shadow-md rounded-lg border border-gray-200 p-6 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Price <span
                            class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Estimated Duration (min) <span
                            class="text-red-500">*</span></label>
                    <input type="number" wire:model.defer="estimated_duration"
                        class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" />
    ...
    <div class="card bg-base-100 shadow-md rounded-lg border border-gray-200 overflow-hidden">
        <table class="min-w-full table-auto text-xs">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Service
                    </th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        Description</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Price
                    </th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        Duration</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Status
                    </th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Actions
                    </th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($serviceTypes as $type)
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            <span
                                class="inline-flex items-center justify-center h-7 w-7 rounded-full bg-gray-200 text-black font-bold text-xs">
                                {{ $rowNumber }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="font-semibold text-black text-xs">{{ $type->name }}</div>
                            <div class="text-[10px] text-gray-500">ID: #S{{ str_pad($type->id, 3, '0', STR_PAD_LEFT) }}
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3 space-x-2">
                            <button wire:click="edit({{ $type->id }})"
                                class="btn btn-ghost btn-xs text-primary hover:text-primary-dark" title="Edit">
                                <i class="fas fa-edit w-4 h-4 text-black"></i>
                            </button>
                            <button wire:click="confirmDelete({{ $type->id }})"
                                class="btn btn-ghost btn-xs text-red-500 hover:text-red-700" title="Delete">
        ...
        <div class="flex items-center justify-between px-6 py-3 bg-gray-50 border-t border-gray-200">
            ...
            <div class="text-sm text-gray-700">Showing 1-{{ count($serviceTypes) }} of {{ count($serviceTypes) }}
            </div>
        </div>
    </div>
    ...
    <x-ui-modal :show="$serviceTypeId && !$editMode" closeMethod="resetForm" maxWidth="sm">
        ...
        <x-modal-body>
            <div class="flex items-start gap-4">
                ...
                <div>
                    <p class="text-sm text-gray-700">Are you sure you want to delete this service type? This action
                        cannot be undone.</p>
                </div>
            </div>
        </x-modal-body>
