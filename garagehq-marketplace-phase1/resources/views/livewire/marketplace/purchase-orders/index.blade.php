                    ...
                    <tr>
                        ...
                        <td class="text-right whitespace-nowrap">
                            @if ($po->status->value === 'draft')
                                <button class="btn btn-xs btn-primary" wire:click="send({{ $po->id }})">Send</button>
                            @endif
                            @if ($po->status->isReceivable())
                                <a class="btn btn-xs btn-accent" href="{{ route('marketplace.purchase-orders.receive', $po) }}">Receive</a>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="text-center text-base-content/60 py-6">No purchase orders.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
